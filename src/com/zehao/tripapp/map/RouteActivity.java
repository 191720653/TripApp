...
public class RouteActivity extends Activity implements BaiduMap.OnMapClickListener, OnGetRoutePlanResultListener {
	...
	public class MyLocationListenner implements BDLocationListener {
        ...
        public void onReceiveLocation(BDLocation location) {
			...
			Toast.makeText(RouteActivity.this, "当前定位的返回值是："+locType, Toast.LENGTH_SHORT).show();
			...
			if (locType == BDLocation.TypeGpsLocation) {//
				...
				Toast.makeText(RouteActivity.this, addrStr,
						Toast.LENGTH_SHORT).show();
			}
			...
			// 不在景区内
			if(!city.equals("中山市")){
				Toast.makeText(RouteActivity.this, "当前不在景区内！", Toast.LENGTH_SHORT).show();
				return;
			}
			// 构造定位数据
    ...
    public void forLocation(View view) {
    	if(isFirstLoc){
    		...
    		isFirstLoc = false;
    		switchLoc = true;
    		Toast.makeText(RouteActivity.this, "定位已经开启！", Toast.LENGTH_SHORT).show();
    	}else if(switchLoc){
    		mLocClient.stop();
    		Toast.makeText(RouteActivity.this, "定位已经关闭！", Toast.LENGTH_SHORT).show();
    		switchLoc = false;
    	}else if(!switchLoc){
    		mLocClient.start();
    		Toast.makeText(RouteActivity.this, "定位已经开启！", Toast.LENGTH_SHORT).show();
    ...
    public void forZoomMap(View view) {
        ...
        if (view.getId() == R.id.button_zoom_add) {
            if(zoom_level<=18){
                ...
                Toast.makeText(RouteActivity.this, "已经放至最大！", Toast.LENGTH_SHORT).show();
            }
        } else if (view.getId() == R.id.button_zoom_sub) {
        	if(zoom_level>4){
                mBaidumap.setMapStatus(MapStatusUpdateFactory.zoomOut());
            }else{
                Toast.makeText(RouteActivity.this, "已经放至最小！", Toast.LENGTH_SHORT).show();
            }
        }
    }
